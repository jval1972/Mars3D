...
type
  optionsopenglfog_e = (
    ...
    od_glf_filler3,
    od_glf_filler4,
    od_glf_use_underwater_fog,
    od_glf_underwater_fog_density,
    od_glf_filler5,
    od_glf_filler6,
    optglfog_end
  );
...
//==============================================================================
//
// M_ChangeUnderwaterFogDensity
//
//==============================================================================
procedure M_ChangeUnderwaterFogDensity(choice: integer);
begin
  _ChangeFogDensity(choice, underwater_fog_density);
end;
...
begin
  ...
  M_DrawThermo(
    OptionsDisplayOpenGLFogDef.x, OptionsDisplayOpenGLFogDef.y + OptionsDisplayOpenGLFogDef.itemheight * (Ord(od_glf_underwater_fog_density) + 1),
    25, _fog_thermo_index(underwater_fog_density), NUMFOGDENSITIES);
end;
{$ENDIF}
...
//OptionsDisplayOpenGLFogMenu
  ...
  inc(pmi);
  pmi.status := 1;
  pmi.name := '!Underwater sector fog';
  pmi.cmd := 'use_underwater_fog';
  pmi.routine := @M_BoolCmd;
  pmi.pBoolVal := @use_underwater_fog;
  pmi.alphaKey := 'u';

  inc(pmi);
  pmi.status := 2;
  pmi.name := '!Underwater fog density';
  pmi.cmd := '';
  pmi.routine := @M_ChangeUnderwaterFogDensity;
  pmi.pBoolVal := nil;
  pmi.alphaKey := 'u';
...
//OptionsDisplayOpenGLFogDef
  ...
  OptionsDisplayOpenGLFogDef.x := DEF_MENU_ITEMS_START_X;
  OptionsDisplayOpenGLFogDef.y := DEF_MENU_ITEMS_START_Y;
  OptionsDisplayOpenGLFogDef.lastOn := 0; // last item user was on in menu
  OptionsDisplayOpenGLFogDef.itemheight := SMALLLINEHEIGHT;
  OptionsDisplayOpenGLFogDef.flags := FLG_MN_TEXTUREBK or FLG_MN_DRAWITEMON;
{$ENDIF}
